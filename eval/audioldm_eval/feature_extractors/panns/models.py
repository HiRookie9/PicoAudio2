...
class Cnn14(nn.Module):
    def __init__(
        ...
        if not os.path.exists("/hpc_stor03/sjtu_home/zeyu.xie/.cache/panns/Cnn14_16k_mAP=0.438.pth"):
            ...
            os.makedirs("checkpoint/panns", exist_ok=True)
            """
            os.system(
                "wget -P checkpoint/panns %s"
        ...
        if sample_rate == 16000:
            #state_dict = torch.load("/hpc_stor03/sjtu_home/zeyu.xie/.cache/panns/Cnn14_16k_mAP=0.438.pth")
            state_dict = torch.load("/hpc_stor03/sjtu_home/zeyu.xie/.cache/panns/Cnn14_16k_mAP=0.438.pth", map_location=torch.device('cpu'))
            #state_dict = torch.load("checkpoint/panns/Cnn14_16k_mAP=0.438.pth")
            self.load_state_dict(state_dict["model"])
        elif sample_rate == 32000:
            state_dict = torch.load("checkpoint/panns/Cnn14_mAP=0.431.pth")
            self.load_state_dict(state_dict["model"])
