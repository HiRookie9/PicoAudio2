...
def load_audio_task(fname):
    try:
        # librosa会直接返回float32且[-1.0, 1.0]，并自动转单声道
        wav_data, sr = librosa.load(fname, sr=None, mono=True)
    except Exception as e:
        print(e)
        wav_data = np.zeros(160000, dtype=np.float32)
        sr = 16000
...
class FrechetAudioDistance:
    ...
    def __get_model(self, use_pca=False, use_activation=False):
        ...
        self.model = torch.hub.load("/hpc_stor03/sjtu_home/zeyu.xie/.cache/torch/hub/harritaylor_torchvggish_master", "vggish", source='local')
        #self.model = torch.load("/hpc_stor03/sjtu_home/zeyu.xie/.cache/torch/hub/checkpoints/vggish_pca_params-970ea276.pth")
        if not use_pca:
