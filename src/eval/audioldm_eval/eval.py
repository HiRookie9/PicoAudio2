...
class EvaluationHelper:
    ...
    def calculate_lsd(self, pairedloader, same_name=True, time_offset=160 * 7):
        ...
        for idx, (_, _, filename, (audio1, audio2)) in enumerate(tqdm(pairedloader, leave=False)):
    ...
    def calculate_metrics(self, generate_files_path, groundtruth_path, same_name, limit_num=None):
        ...
        gt_dataset = WaveDataset(
            groundtruth_path,
            self.sampling_rate,
            limit_num=limit_num,
        )
        print("WaveDataset length:", len(gt_dataset))
        ...
        print("resultloader length:", len(resultloader))
        """
        ...
        metric_lsd = self.calculate_lsd(pairedloader, same_name=False)
